...
export function OCREngine({ customIntro }: { customIntro?: string }) {
  ...
  const processImage = async (imgSrc: string) => {
    ...
    try {
      ...
      const { data: { text: resultText } } = await worker.recognize(imgSrc);
      setText(resultText);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6" onPaste={handlePaste} data-testid="ocr-engine">
      {customIntro && <p className="text-muted-foreground text-center mb-6">{customIntro}</p>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          {image ? (
            <>
              <img src={image} alt="Preview" className="max-w-full max-h-full object-contain rounded-md" />
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                <Button variant="secondary" onClick={() => fileInputRef.current?.click()} data-testid="btn-change-image">
                  Change Image
                </Button>
              </div>
            </>
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Drag & drop, paste (Ctrl+V), or click to browse</p>
              </div>
              <Button onClick={() => fileInputRef.current?.click()} data-testid="btn-browse-files">Browse Files</Button>
            </div>
          )}
          <input 
            type="file" 
          ...
          <div className="flex-1 p-0 relative">
            {isProcessing ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm z-10">
                <div className="text-primary font-medium mb-2">Scanning Image... {progress}%</div>
                <div className="w-48">
